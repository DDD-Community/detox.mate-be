...
- 대상 `memberId`가 `groupId` 그룹의 활성 멤버인지 확인한다.
...
| 404 | `GROUP_MEMBER_NOT_FOUND` | 대상 멤버가 해당 그룹의 활성 멤버가 아님 |
...
이번 PR에서는 mock API 문서화 이후 실제 탈퇴 동작을 연결하는 최소 범위만 다룬다.

- `DELETE /groups/{groupId}/members/me`가 호출하는 `GroupService.withdrawGroup(groupId, userId)`에 실제 탈퇴 흐름을 연결한다.
...
- 최신 챌린지 기준은 `GroupChallengeService.getLatestChallenge(groupId)`와 동일하게 둔다.
- 기존 `leaveGroup(groupId, userId)`에 실제 로직이 남아 있다면 공통 private 메서드로 모아 중복을 줄인다.
- 변경 후 `GroupServiceTest`, `GroupControllerTest`에서 탈퇴 API가 204를 반환하고 service가 실제 탈퇴 흐름을 타는지 검증한다.
...
- [ ] 조회 대상 `memberId`가 해당 그룹의 활성 멤버인지 확인
- [ ] 멤버 기본 정보 조회
- [ ] 현재 목표 시간 목록 조회
- [ ] D+ 계산
- [ ] 전체 달성률 계산
...
- [ ] 그룹 존재 여부 확인
- [ ] 내가 해당 그룹의 활성 멤버인지 확인
- [ ] `GroupMember`를 `LEFT` 처리
- [ ] `GroupMember.leftAt` 기록
- [ ] 최신 챌린지 참가 상태를 `WITHDRAWN` 처리
- [ ] 최신 챌린지 참가자의 `withdrawnAt` 기록
- [ ] 과거 챌린지 참가 상태는 변경하지 않음
- [ ] `DELETE /groups/{groupId}/members/me`가 실제 탈퇴 흐름을 호출하도록 연결
- [ ] 기존 `leaveGroup` 로직과 중복되지 않도록 공통화
- [ ] 탈퇴 후 그룹이 0명일 때 그룹/챌린지 처리 정책 확인
- [ ] 회원 탈퇴와 동일한 그룹 이탈 흐름으로 재사용할지 정리
- [ ] 인증 생성 API 권한 검증은 이번 PR 범위에서 제외
- [ ] 챌린지 조회 권한 조건 변경은 이번 PR 범위에서 제외
- [ ] 그룹/멤버 목록 필터링 변경은 이번 PR 범위에서 제외

현재 상태:

- mock API 계약만 잡혀 있다.
- 실제 탈퇴 기능은 아직 구현되지 않았다.
...
- [x] 204 응답 contract 문서화
- [ ] 소셜 로그인 연결 정보 삭제
- [ ] refresh token/session 삭제
- [ ] user 삭제
- [ ] 활성 그룹이 있을 때 그룹 탈퇴와 동일한 처리 수행
- [ ] FCM token 삭제 여부 결정 및 처리
- [ ] 이미 탈퇴/없는 사용자 케이스 정책 정리
- [ ] 관련 도메인 데이터 보존/삭제 정책 정리

현재 상태:

- mock API 계약만 잡혀 있다.
- 실제 회원 탈퇴 기능은 이 API에는 아직 연결하지 않았다.
