...
class PokeControllerTest {
    ...
    private static final String POKE_URL = "/group-challenges/{groupChallengeId}/members/{targetUserId}/poke";
    ...
    void poke_persistsPokeAndReturns204() throws Exception {
        // when & then
        mockMvc.perform(post(POKE_URL, GROUP_CHALLENGE_ID, targetUserId)
    ...
    void poke_duplicateToday_returns400() throws Exception {
        // given
        pokeRepository.save(Poke.create(GROUP_CHALLENGE_ID, currentUserId, targetUserId, LocalDate.now()));
    ...
    void poke_allowsDifferentTargetUser() throws Exception {
        ...
        // when & then
        mockMvc.perform(post(POKE_URL, GROUP_CHALLENGE_ID, otherTargetUserId)
                        .header(HttpHeaders.AUTHORIZATION, bearer(currentUserId)))
                .andExpect(status().isNoContent());
    ...
    void poke_self_returns400() throws Exception {
        mockMvc.perform(post(POKE_URL, GROUP_CHALLENGE_ID, currentUserId)
                        .header(HttpHeaders.AUTHORIZATION, bearer(currentUserId)))
                .andExpect(status().isBadRequest());
